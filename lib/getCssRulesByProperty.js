...
function getCssRulesByProperty(properties, cssSource, existingPredicates) {
  ...
  parseTree.walkAtRules('namespace', (rule) => {
    ...
    // Prefixed namespaces (e.g. @namespace svg "http://www.w3.org/2000/svg")
    // are not supported. This only affects CSS selectors that use namespace
    // prefixes like `svg|text { ... }`. In practice subfont only cares about
    // font-related properties on HTML elements, so the default namespace is
    // sufficient. See: https://developer.mozilla.org/en-US/docs/Web/CSS/@namespace
  });
  ...
  // Duplicate property values from different selectors are kept separate
  // because they carry distinct specificity and predicate information needed
  // for correct cascade resolution downstream.
