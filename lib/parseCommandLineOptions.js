module.exports = function parseCommandLineOptions(argv) {
  ...
  yargs
    ...
    .options('source-maps', {
      alias: ['sourceMaps'],
      describe:
        'Preserve CSS source maps through subfont processing. Off by default for faster execution',
      type: 'boolean',
      default: false,
    })
    .options('skip-source-map-processing', {
      // Backwards compat: --skip-source-map-processing is equivalent to --no-source-maps
      type: 'boolean',
      hidden: true,
    })
  ...
  const { _: inputFiles, skipSourceMapProcessing, ...rest } = yargs.argv;

  // Backwards compat: --skip-source-map-processing means --no-source-maps
  // Only apply when --source-maps was not explicitly provided by the user.
  if (skipSourceMapProcessing !== undefined && rest.sourceMaps === false) {
    // sourceMaps defaults to false, so we can only distinguish an explicit
    // --source-maps / --no-source-maps from the default by checking whether
    // the raw argv contained it.
    const rawArgv = argv || process.argv.slice(2);
    const hasExplicitSourceMaps = rawArgv.some(
      (arg) =>
        arg === '--source-maps' ||
        arg === '--no-source-maps' ||
        arg === '--sourceMaps' ||
        arg === '--no-sourceMaps' ||
        arg.startsWith('--source-maps=') ||
        arg.startsWith('--sourceMaps=')
    );
    if (!hasExplicitSourceMaps) {
      rest.sourceMaps = !skipSourceMapProcessing;
    }
  }
