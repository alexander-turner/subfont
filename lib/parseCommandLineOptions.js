module.exports = function parseCommandLineOptions(argv) {
  ...
  yargs
    ...
    .options('cache', {
      ...
      default: false,
      coerce(val) {
        // --cache (no value) comes through as empty string; treat as boolean true
  ...
  // Only apply when --source-maps was not explicitly provided (undefined).
  if (skipSourceMapProcessing !== undefined && rest.sourceMaps === undefined) {
    rest.sourceMaps = !skipSourceMapProcessing;
  }

  // Apply default (false) when neither flag was provided
  if (rest.sourceMaps === undefined) {
    rest.sourceMaps = false;
  }
