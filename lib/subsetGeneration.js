...
async function getSubsetsForFontUsage(
  ...
  for (const [fontUrl, fontUsage] of canonicalFontUsageByUrl) {
    ...
    let text = fontUsage.text;

    // When font-feature-settings or font-variant-* CSS properties are used,
    // include all original codepoints from the font. This ensures HarfBuzz
    // layout closure can fully traverse GSUB feature chains (aalt, ss01-ss05,
    // salt, etc.) and preserve all alternate glyphs.
    if (fontUsage.hasFontFeatureSettings && fontBuffer) {
      const fontInfo = await getFontInfo(fontBuffer);
      if (fontInfo.characterSet && fontInfo.characterSet.length > 0) {
        const allChars = fontInfo.characterSet
