...
async function getSubsetsForFontUsage(
  ...
  const numSubsetOps = Object.keys(subsetPromiseMap).length;
  const subsetStart = Date.now();
  await Promise.all(Object.values(subsetPromiseMap));
  const subsetDuration = Date.now() - subsetStart;
  // eslint-disable-next-line no-console
  console.log(
    `[subfont timing] Font subsetting: ${subsetDuration}ms (${canonicalFontUsageByUrl.size} unique fonts, ${numSubsetOps} subset operations)`
  );
