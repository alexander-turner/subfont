...
async function getSubsetsForFontUsage(
  ...
  if (instance) {
    const fontUrls = [...canonicalFontUsageByUrl.keys()].filter(
      (url) => fontAssetsByUrl.has(url)
    );
  ...
  for (const [fontUrl, fontUsage] of canonicalFontUsageByUrl) {
    ...
    for (const targetFormat of formats) {
      ...
      subsetPromiseMap[promiseId] = subsetPromiseMap[promiseId].then(
        (subsetBuffer) => {
          if (!subsetBuffer) {
            return subsetBuffer;
          }
