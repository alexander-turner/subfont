...
module.exports = async function subfont(
  ...
  if (debug) {
    // Aggregate across pages: one entry per unique fontUrl. Codepoint unions
    // and subset sizes are per-font (not per-page), so the only per-page
    // variation worth surfacing is which pages pulled the font in.
    const byFontUrl = new Map();
    for (const { assetFileName, fontUsages } of fontInfo) {
      for (const fu of fontUsages) {
        const key =
          fu.fontUrl ||
          `[inline] ${fu.props['font-family']}/${fu.props['font-weight']}/${fu.props['font-style']}`;
        let entry = byFontUrl.get(key);
        if (!entry) {
          entry = {
            fontUrl: fu.fontUrl,
            props: fu.props,
          ...
          byFontUrl.set(key, entry);
        }
        entry.pages.push(assetFileName);
      }
    }
    const SAMPLE_PAGES = 5;
    const aggregated = [...byFontUrl.values()].map((entry) => {
      const { pages, ...rest } = entry;
      const samples = pages.slice(0, SAMPLE_PAGES);
      return {
        ...rest,
        pageCount: pages.length,
        pages:
          pages.length > SAMPLE_PAGES
            ? [...samples, `…and ${pages.length - SAMPLE_PAGES} more`]
            : samples,
      };
    });
    log(util.inspect(aggregated, false, 99));
  }
