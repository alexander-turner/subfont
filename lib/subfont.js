...
module.exports = async function subfont(
  ...
  if (debug) log(`[subfont timing] loadAssets: ${Date.now() - phaseStart}ms`);

  phaseStart = Date.now();
  await assetGraph.populate({
    followRelations: followRelationsQuery,
  });
  if (debug) log(`[subfont timing] populate (initial): ${Date.now() - phaseStart}ms`);
  ...
  if (debug) log(`[subfont timing] subsetFonts total: ${Date.now() - phaseStart}ms`);
  ...
  if (debug) log(`[subfont timing] post-subsetFonts processing: ${Date.now() - phaseStart}ms`);
  ...
  if (debug) log(`[subfont timing] writeAssetsToDisc: ${Date.now() - phaseStart}ms`);
  ...
  if (debug) log(`[subfont timing] output reporting: ${Date.now() - phaseStart}ms`);
