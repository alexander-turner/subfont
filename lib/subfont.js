...
module.exports = async function subfont(
  ...
  if (debug)
    log(`[subfont timing] populate (initial): ${outerTimings['populate (initial)']}ms`);
  ...
  if (debug)
    log(`[subfont timing] subsetFonts total: ${subsetFontsTotal}ms`);
  ...
  if (debug)
    log(`[subfont timing] writeAssetsToDisc: ${outerTimings.writeAssetsToDisc}ms`);
  ...
  if (debug)
    log(`[subfont timing] output reporting: ${outerTimings['output reporting']}ms`);

  // Always print timing summary so it's visible in CI logs
  outerTimings.subsetFontsTotal = subsetFontsTotal;
  const totalElapsed =
  ...
  log(`  loadAssets:                ${(outerTimings.loadAssets || 0).toLocaleString().padStart(8)}ms`);
  log(`  populate (initial):        ${(outerTimings['populate (initial)'] || 0).toLocaleString().padStart(8)}ms`);
  log(`  subsetFonts total:         ${subsetFontsTotal.toLocaleString().padStart(8)}ms`);
  if (subsetTimings) {
    const details = subsetTimings.collectTextsByPageDetails || {};
    log(`    collectTextsByPage:      ${(subsetTimings.collectTextsByPage || 0).toLocaleString().padStart(8)}ms`);
    if (details['Stylesheet precompute'] !== undefined)
      log(`      Stylesheet precompute: ${details['Stylesheet precompute'].toLocaleString().padStart(8)}ms`);
    if (details['Full tracing'] !== undefined)
      log(`      Full tracing:          ${details['Full tracing'].toLocaleString().padStart(8)}ms`);
    if (details['Fast-path extraction'] !== undefined)
      log(`      Fast-path extraction:  ${details['Fast-path extraction'].toLocaleString().padStart(8)}ms`);
    if (details['Per-page loop'] !== undefined)
      log(`      Per-page loop:         ${details['Per-page loop'].toLocaleString().padStart(8)}ms`);
    if (details['Post-processing total'] !== undefined)
      log(`      Post-processing:       ${details['Post-processing total'].toLocaleString().padStart(8)}ms`);
    log(`    codepoint generation:    ${(subsetTimings['codepoint generation'] || 0).toLocaleString().padStart(8)}ms`);
    log(`    getSubsetsForFontUsage:  ${(subsetTimings.getSubsetsForFontUsage || 0).toLocaleString().padStart(8)}ms`);
    log(`    insert subsets loop:     ${(subsetTimings['insert subsets loop'] || 0).toLocaleString().padStart(8)}ms`);
    log(`    inject font-family:      ${(subsetTimings['inject subset font-family'] || 0).toLocaleString().padStart(8)}ms`);
  }
  log(`  post-subsetFonts:          ${(outerTimings['post-subsetFonts processing'] || 0).toLocaleString().padStart(8)}ms`);
  log(`  writeAssetsToDisc:         ${(outerTimings.writeAssetsToDisc || 0).toLocaleString().padStart(8)}ms`);
  log(`  output reporting:          ${(outerTimings['output reporting'] || 0).toLocaleString().padStart(8)}ms`);
  log(`  ─────────────────────────────────`);
