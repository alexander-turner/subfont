...
module.exports = async function subfont(
  ...
  // Returns true for ENOENT errors on extensionless file paths.
  // Sites using clean URLs (e.g., /about instead of /about.html) produce
  // these warnings when assetGraph can't resolve them literally.
  ...
  if (silent) {
    ...
    // Use logEvents for its transform-timing output, but filter ENOENT
    // warnings for extensionless URLs before they reach the console.
    // We monkey-patch emit because logEvents registers its own warn
    // listener that we can't prevent from firing.
  ...
  const { fontInfo, timings: subsetTimings } = await subsetFonts(assetGraph, {
    ...
    cacheDir:
      cache && typeof cache === 'string' && cache.length > 0
        ? cache
        : cache && rootUrl && rootUrl.startsWith('file:')
          ? pathModule.join(urlTools.fileUrlToFsPath(rootUrl), '.subfont-cache')
          : null,
  });
  ...
  if (dryRun) {
    // Show a comprehensive preview of what would be written
    log('\n═══ Dry Run Preview ═══');
    ...
    for (const asset of assetsToWrite) {
      ...
      // Show individual files for subset assets (CSS + fonts)
    ...
    // Show generated subset CSS summary
