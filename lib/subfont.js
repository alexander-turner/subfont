...
module.exports = async function subfont(
  ...
  if (skipSourceMapProcessing) {
    log(
      'Skipping CSS source map processing for faster execution. Use --no-skip-source-map-processing to preserve source maps.'
    );
  }
  ...
  if (debug)
    log(
      `[subfont timing] post-subsetFonts processing: ${
        Date.now() - phaseStart
      }ms`
    );
  ...
  for (const { assetFileName, fontUsages } of fontInfo) {
    ...
    const fontUsagesByFontFamily = Object.groupBy(
      fontUsages,
