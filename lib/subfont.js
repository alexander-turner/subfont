...
module.exports = async function subfont(
  ...
  let _subfontPhaseStart = Date.now();
  await assetGraph.loadAssets(inputUrls);
  log(`[subfont timing] loadAssets: ${Date.now() - _subfontPhaseStart}ms`);

  _subfontPhaseStart = Date.now();
  await assetGraph.populate({
    followRelations: followRelationsQuery,
  });
  log(`[subfont timing] populate (initial): ${Date.now() - _subfontPhaseStart}ms`);
  ...
  log(`[subfont timing] subsetFonts total: ${Date.now() - _subfontPhaseStart}ms`);
  ...
  log(`[subfont timing] post-subsetFonts processing: ${Date.now() - _subfontPhaseStart}ms`);
  ...
  log(`[subfont timing] writeAssetsToDisc: ${Date.now() - _subfontPhaseStart}ms`);
  ...
  log(`[subfont timing] output reporting: ${Date.now() - _subfontPhaseStart}ms`);
