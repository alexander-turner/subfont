...
module.exports = async function subfont(
  ...
  // cssnano (run via assetgraph minify) pulls in postcss plugins that call
  // browserslist(null, { path: __dirname }). Browserslist walks up the dir
  // tree looking for a file literally named `browserslist`, and pnpm ships
  // shell-script shims at `.bin/browserslist` that can confuse that lookup,
  // producing 'Unknown browser query basedir=$(...)' noise. Pinning the env
  // var short-circuits the config search with a sane default.
  ...
  if (strict && sawWarning) {
    // In non-silent mode, assetgraph's logEvents also exits on warnings it
    // emits during transforms. This guard catches warnings in --silent mode
    // and any warning that slipped past logEvents.
    throw new Error(
      'subfont: --strict was set and one or more warnings were emitted; failing the run.'
    );
  }
  return assetGraph;
};
