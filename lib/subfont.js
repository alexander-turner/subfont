...
module.exports = async function subfont(
  ...
  // Suppress ENOENT warnings for extensionless file:// URLs.
  // Sites using clean URLs (e.g., /about instead of /about.html) produce
  // these warnings when assetGraph tries to resolve links literally.
  // These are harmless noise — the files exist with .html extensions but
  // assetGraph's file resolver doesn't try alternatives.
  const origEmit = assetGraph.emit.bind(assetGraph);
  assetGraph.emit = function (event, err, ...rest) {
    if (
      ...
      return false; // Suppress extensionless ENOENT warnings
    }
    return origEmit(event, err, ...rest);
  };
