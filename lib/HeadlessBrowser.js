  ...
  async tracePage(htmlAsset) {
    ...
    page.on('pageerror', (err) => {
      if (err instanceof Error && err.stack) {
        // Normalize the stack: convert "at <anonymous> (url:line:col)" to
        // a concise format matching the old string-based pageerror output
        const formatted = err.stack.replace(
          /\s+at <anonymous> \((.+?)\)/g,
          '\n    at $1'
        );
