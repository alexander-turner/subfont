...
// Default timeout for page operations (navigation, script evaluation, etc.)
const PAGE_TIMEOUT_MS = 120000; // 2 minutes
...
async function downloadOrLocatePreferredBrowserRevision() {
  ...
  try {
    ...
    throw new Error(
      `Failed to launch browser (executable: ${executablePath}): ${err.message}`
    );
  }
}
  ...
  async tracePage(htmlAsset) {
    ...
    try {
      ...
      page.on('request', (request) => {
        ...
        // If the URL shares the same origin as the base but wasn't found
        // in the graph, serve a synthetic 404 so the page can continue
        // loading without hitting the real network.
        if (url.startsWith(baseUrl)) {
          request.respond({ status: 404, body: '' });
          return;
        }
        // For cross-origin requests to third-party domains, let them
        // through when a real server is likely available (non-file: root),
        // otherwise abort to avoid DNS look-ups / timeouts.
