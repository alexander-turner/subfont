...
async function collectTextsByPage(
  ...
  const overallPhase = logPhaseStart(console, debug, 'collectTextsByPage');

  // Pre-compute stylesheet results for all pages
  const stylesheetPrecompute = logPhaseStart(
    console,
    debug,
    'Stylesheet precompute'
  );
  const pageData = [];
  ...
  const fullTracing = logPhaseStart(
    console,
    debug,
