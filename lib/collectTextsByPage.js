...
// Build a structured cache key from font-face declarations.
// Uses JSON.stringify to avoid delimiter-collision risks.
...
// Standalone function extracted from collectTextsByPage closure.
// Snaps each globalTextByProps entry against font-face declarations
// to determine which font URL and properties each text segment maps to.
//
// Parameters are explicit instead of captured from outer scope:
//   declarations     — the accumulatedFontFaceDeclarations for this page group
//   globalTextByProps — the global array of {text, props, htmlOrSvgAsset} entries
function computeSnappedGlobalEntries(declarations, globalTextByProps) {
  ...
  for (const textAndProps of globalTextByProps) {
    ...
    // Build a structured key from the props that affect snapping (not text)
...
// Standalone function extracted from collectTextsByPage closure.
// Builds global font usage templates and per-page indices from
// snapped entries. Mutates declCache[declKey] in place.
//
// Parameters are explicit instead of captured from outer scope:
//   declKey                        — cache key for this declarations set
//   accumulatedFontFaceDeclarations — the font-face declarations
//   declCache                      — the Map holding cached data per declKey
//   globalTextByProps              — (unused directly, but snappedEntries reference it)
//   text                           — the --text option passed to subfont (may be undefined)
function getOrComputeGlobalFontUsages(
  ...
  for (const fontUrl of allFontUrls) {
    ...
    allTexts.push(...fontEntries.map((e) => e.textAndProps.text));
    ...
    const hasOutOfBoundsAnimationTimingFunction = fontEntries.some((e) =>
        ...
        for (const entry of repTextByProps) {
          ...
          if (family) {
            ...
            for (const fam of cssFontParser.parseFontFamily(family)) {
              seenVariantKeys.add(
                JSON.stringify([fam.toLowerCase(), weight, style, stretch])
              );
            }
          }
        }
