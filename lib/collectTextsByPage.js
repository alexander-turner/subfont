...
async function collectTextsByPage(
  ...
  if (debug) console.log('[subfont timing] collectTextsByPage started');
  ...
  try {
    if (useWorkerPool) {
      const maxWorkers = concurrency || Math.min(os.cpus().length, 8);
      ...
      try {
        ...
        const tracePromises = pagesNeedingFullTrace.map(async (pd) => {
          ...
          tracedCount++;
          if (showProgress && tracedCount % 50 === 0) {
            console.log(
