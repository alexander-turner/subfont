...
// clamped to [1, 10]. For a 500-page run this prints every 50 pages;
// for 30 pages, every 3. Avoids a long silent stretch on large runs
// and "every page" spam on small ones.
function progressStep(totalPages) {
  return Math.max(1, Math.min(10, Math.ceil(totalPages / 10)));
}
...
async function tracePages(
  ...
  if (useWorkerPool) {
    ...
    try {
      ...
      const step = progressStep(totalPages);
      const showProgress = totalPages >= 5 && console;
      if (showProgress) {
        console.log(
      ...
      const tracePromises = pagesNeedingFullTrace.map(async (pd) => {
        ...
        if (debug && console) {
          console.log(
            `[subfont timing]   traced [${tracedCount}/${totalPages}] ${pd.htmlOrSvgAsset.urlOrDescription} in ${Date.now() - pageStart}ms`
          );
        }
        if (
          showProgress &&
          tracedCount % step === 0 &&
          tracedCount < totalPages
        ) {
      ...
      if (showProgress) {
        console.log(`  Tracing fonts: ${totalPages}/${totalPages} pages done.`);
      }
    } finally {
      await pool.destroy();
    }
    ...
    for (let pi = 0; pi < totalPages; pi++) {
      ...
      if (debug && console) {
        console.log(
          `[subfont timing]   traced [${pi + 1}/${totalPages}] ${pd.htmlOrSvgAsset.urlOrDescription} in ${Date.now() - pageStart}ms`
        );
      }
      if (showProgress && (pi + 1) % step === 0 && pi + 1 < totalPages) {
