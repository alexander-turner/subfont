...
function planTracing(pageData, hasHeadlessBrowser) {
  ...
  return { pagesNeedingFullTrace, fastPathPages, pagesByStylesheetKey };
}
...
function assembleTracingResults(
  pageData,
  htmlOrSvgAssetTextsWithProps,
  globalTextByProps
) {
...
async function collectTextsByPage(
  ...
  const { pagesNeedingFullTrace, fastPathPages, pagesByStylesheetKey } =
