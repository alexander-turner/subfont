      ...
      for (const char of fontUsage.pageText) {
        ...
        if (isMissing) {
          ...
          if (entry.locations.length < MAX_LOCATIONS_PER_GLYPH) {
            ...
            let firstIdx = -1;
            if (char.length > 0) {
              const lastReported = entry.locations[entry.locations.length - 1];
              const searchFrom =
                lastReported && lastReported._idx !== undefined
                  ? lastReported._idx + char.length
                  : 0;
              firstIdx = sourceText.indexOf(char, searchFrom);
            }
            if (firstIdx !== -1) {
              if (!linesAndColumns) {
                linesAndColumns = new LinesAndColumns(sourceText);
              }
              const position = linesAndColumns.locationForIndex(firstIdx);
              entry.locations.push({
                _idx: firstIdx,
                text: `${htmlOrSvgAsset.urlOrDescription}:${position.line + 1}:${position.column + 1}`,
              });
            } else if (entry.locations.length === 0) {
              entry.locations.push({
                text: `${htmlOrSvgAsset.urlOrDescription} (generated content)`,
              });
            }
          }
          missedAny = true;
  ...
  if (collapsed.size > 0) {
    const errorLog = [...collapsed.values()].map(
      ({ char, fontUsage, locations, count }) => {
        const locationTexts = locations.map((l) => l.text);
        const suffix =
          count > locationTexts.length
            ? ` (+${count - locationTexts.length} more)`
            : '';
        ...
        })${countPrefix} at ${locationTexts.join(', ')}${suffix}`;
      }
    );
