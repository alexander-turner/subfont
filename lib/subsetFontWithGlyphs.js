...
async function subsetFontWithGlyphs(
  ...
  try {
    ...
    // Route woff2 compression through the worker pool (each worker has
    // its own wawoff2 WASM instance).  Non-woff2 formats use JS-based
    // converters that are safe to call concurrently in the main thread.
    return targetFormat === 'woff2'
      ? getPool().convert(subsetFont, targetFormat, 'truetype')
...
// Pre-warm both pools: overlap WASM compilation and worker startup
// with font tracing work so neither appears on the critical path.
subsetFontWithGlyphs.warmup = () => {
  getPool()
    ._ensureInit()
    .catch(() => {});
  return initPool();
