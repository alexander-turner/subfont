...
/**
 * Check if the buffer is already TrueType/OpenType (no conversion needed).
 */
function isTrueType(buf) {
  if (buf.length < 4) return false;
  const sig = buf.readUInt32BE(0);
  return sig === 0x00010000 || sig === 0x74746366; // TrueType or ttcf
}
...
async function subsetFontWithGlyphs(
  ...
  const ttf = isTrueType(originalFont)
    ? originalFont
