...
// Run only the harfbuzz subsetting on a specific WASM instance.
// Returns raw TrueType subset buffer (independent copy, safe to use
// after WASM resources are freed in the finally block).
async function doHarfbuzzSubset(
  exports,
  ttf,
  text,
  { glyphIds, variationAxes } = {}
) {
  ...
  try {
    ...
    return extractSubsetFont(exports, getHeapu8(exports), subset);
  } finally {
...
// wawoff2 is the only concurrency-unsafe dependency in fontverter.
// Other format conversions (woff, truetype) are synchronous or use
// separate native/JS libraries that are fine to call in parallel.
function usesWawoff2(buffer) {
  return buffer.length >= 4 && buffer.toString('ascii', 0, 4) === 'wOF2';
}

// Three-stage pipeline per font:
//   1. sfnt conversion   — serialized only when source is woff2
//   2. harfbuzz subset   — parallel   (independent WASM instances)
//   3. format conversion — serialized only when target is woff2
// Stages 1 and 3 only use convertLimiter when wawoff2 is involved,
// so truetype and woff outputs don't pay serialization overhead.
async function subsetFontWithGlyphs(originalFont, text, options) {
  const pool = await initPool();
  const { targetFormat, glyphIds, variationAxes } = options || {};

  // Stage 1: convert to sfnt
  const ttf = usesWawoff2(originalFont)
    ? await convertLimiter(() => toSfnt(originalFont))
    : await toSfnt(originalFont);

  // Stage 2: harfbuzz subsetting (parallel across pool instances)
  const idx = _nextSlot;
  _nextSlot = (_nextSlot + 1) % pool.length;
  const { exports, limiter } = pool[idx];
  const subsetFont = await limiter(() =>
    doHarfbuzzSubset(exports, ttf, text, { glyphIds, variationAxes })
  );
