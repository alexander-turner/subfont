...
//   1. sfnt conversion   — serialized (wawoff2 not concurrency-safe)
//   2. harfbuzz subset   — parallel   (independent WASM instances)
//   3. format conversion — serialized (wawoff2 not concurrency-safe)
// Each stage runs outside the other stages' locks, so a font in stage 2
// doesn't block another font's stage 1 or 3, and vice versa.
async function subsetFontWithGlyphs(originalFont, text, options) {
  const pool = await initPool();
  const { targetFormat, glyphIds, variationAxes } = options || {};

  // Stage 1: convert to sfnt (serialized — wawoff2 decompression)
  ...
  // Stage 3: convert to target format (serialized — wawoff2 compression)
