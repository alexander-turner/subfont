...
async function compileModule() {
  if (!_compilePromise) {
    _compilePromise = WebAssembly.compile(
      await readFile(require.resolve('harfbuzzjs/hb-subset.wasm'))
    );
  }
  _compiledModule = await _compilePromise;
...
async function subsetFontWithGlyphs(
  ...
  let inst = await acquireInstance();
  const { exports } = inst;
  try {
    ...
    try {
      ...
      const subsetFont = extractSubsetFont(exports, subset);
      // Release the WASM instance before the serialized convert step
      // so other subsetting calls can proceed in parallel.
