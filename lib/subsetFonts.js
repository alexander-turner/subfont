...
async function collectTextsByPage(
  ...
  // Cache stylesheet-dependent results by stylesheet identity key.
  // Pages sharing the same set of CSS assets (very common for multi-page
  // sites like blogs) can reuse gatherStylesheetsWithPredicates output,
  // accumulatedFontFaceDeclarations, and font-face combo validation.
  const stylesheetResultCache = new Map();

  function getStylesheetIdentityKey(asset) {
    const cssAssetIds = [];
    assetGraph.eachAssetPreOrder(asset, traversalRelationQuery, (a) => {
      if (a.type === 'Css') {
        cssAssetIds.push(a.id);
      }
    });
    return cssAssetIds.join('\x1d');
  }

  function getOrComputeStylesheetResults(htmlOrSvgAsset) {
    const key = getStylesheetIdentityKey(htmlOrSvgAsset);
    ...
    // Compute gatherStylesheetsWithPredicates (once per unique stylesheet set)
  ...
  for (const htmlOrSvgAssetTextsWithPropsEntry of htmlOrSvgAssetTextsWithProps) {
    ...
    // Build snappedTexts from subfont-text hints and text param (small, fast)
    const snappedTexts = [];
    ...
    // Add pre-computed snapped global entries with page-specific preload flag
    const pageTextByPropsSet = new Set(textByProps);
    for (const entry of snappedGlobalEntries) {
      snappedTexts.push({
        htmlOrSvgAsset: entry.textAndProps.htmlOrSvgAsset,
        text: entry.textAndProps.text,
        fontVariationSettings: entry.fontVariationSettings,
        fontStyle: entry.fontStyle,
        fontWeight: entry.fontWeight,
        fontStretch: entry.fontStretch,
        animationTimingFunction: entry.animationTimingFunction,
        props: entry.props,
        fontRelations: entry.fontRelations,
        fontUrl: entry.fontUrl,
        preload: pageTextByPropsSet.has(entry.textAndProps),
      });
    }

    // Group by fontUrl and build fontUsages (same logic as groupTextsByFontFamilyProps)
    const filteredTexts = snappedTexts.filter(
      (textByProps) => textByProps && textByProps.fontUrl
    );
    const textsByFontUrl = _.groupBy(filteredTexts, 'fontUrl');

    htmlOrSvgAssetTextsWithPropsEntry.fontUsages = _.map(
      textsByFontUrl,
      (textsPropsArray, fontUrl) => {
        const texts = textsPropsArray.map((obj) => obj.text);
        const preload = textsPropsArray.some((obj) => obj.preload);
        const fontFamilies = new Set(
          textsPropsArray.map((obj) => obj.props['font-family'])
        ...
        const fontStyles = new Set(
          textsPropsArray.map((obj) => obj.fontStyle)
        );
        const fontWeights = new Set(
          textsPropsArray.map((obj) => obj.fontWeight)
        );
        const fontStretches = new Set(
          textsPropsArray.map((obj) => obj.fontStretch)
        );
        const fontVariationSettings = new Set(
          textsPropsArray
            .map((obj) => obj.fontVariationSettings)
            .filter(
              (fvs) => fvs && fvs.toLowerCase() !== 'normal'
            )
        );
        const hasOutOfBoundsAnimationTimingFunction = textsPropsArray.some(
          (obj) =>
            isOutOfBoundsAnimationTimingFunction(obj.animationTimingFunction)
        );
        ...
        for (const relation of textsPropsArray[0].fontRelations) {
          if (relation.to.isLoaded) {
            const size = relation.to.rawSrc.length;
            if (
              smallestOriginalSize === undefined ||
              size < smallestOriginalSize
            ) {
              smallestOriginalSize = size;
              smallestOriginalFormat = relation.to.type.toLowerCase();
            }
        ...
        return {
          ...
          pageText: uniqueChars(
            textsPropsArray
              .filter(
                (textsProps) => textsProps.htmlOrSvgAsset === htmlOrSvgAsset
              )
              .map((obj) => obj.text)
              .join('')
          ),
