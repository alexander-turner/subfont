...
async function subsetFonts(
  ...
  for (const {
    ...
    // When font-display is 'optional', skip preloads — the browser only
    // uses the font if it is already cached, so preloading is wasteful.
    const skipPreload = fontDisplay === 'optional';
    const unsubsettedFontUsagesToPreload = skipPreload
      ? []
      : unsubsettedFontUsages.filter((fontUsage) => fontUsage.preload);
    ...
    for (const fontRelation of cssAsset.outgoingRelations) {
      if (skipPreload || fontRelation.hrefType === 'inline') {
        continue;
      }
      const fontAsset = fontRelation.to;
