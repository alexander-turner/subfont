...
async function collectTextsByPage(
  ...
  function getOrComputeStylesheetResults(htmlOrSvgAsset) {
    ...
    // font-variant-* CSS properties. We scan each stylesheet's text for
    // CSS rule blocks containing these properties, then extract font-family
    // names from the same block. If no font-family is in the same block
    ...
    let fontFamiliesWithFeatureSettings = null; // null = not detected
    for (const swp of stylesheetsWithPredicates) {
      ...
      // Extract CSS rule blocks (between { }) and check each one
      // Match balanced braces at the top level of rule blocks
      const ruleBlockPattern = /\{([^{}]*(?:\{[^{}]*\}[^{}]*)*)\}/g;
      let match;
      while ((match = ruleBlockPattern.exec(swp.text)) !== null) {
        const block = match[1];
        if (
          !featureSettingsPattern.test(block) &&
          !fontVariantPattern.test(block)
        ) {
          continue;
        }
        // This block has font-feature-settings or font-variant-*
        const familyMatch = block.match(/font-family\s*:\s*([^;]+)/i);
        if (familyMatch) {
          if (!fontFamiliesWithFeatureSettings) {
            fontFamiliesWithFeatureSettings = new Set();
          }
          if (fontFamiliesWithFeatureSettings === true) break;
          ...
          // No font-family in this block — conservatively mark all fonts
