...
async function getSubsetsForFontUsage(
  ...
  for (const item of htmlOrSvgAssetTextsWithProps) {
    for (const fontUsage of item.fontUsages) {
      ...
      let numAxesPinned = 0;
      if (instance) {
        let res = variationAxisBoundsCache.get(fontUsage.fontUrl);
        if (!res) {
          ...
          variationAxisBoundsCache.set(fontUsage.fontUrl, res);
        }
...
async function collectTextsByPage(
  ...
  try {
    for (const htmlOrSvgAsset of htmlOrSvgAssets) {
      ...
      if (accumulatedFontFaceDeclarations.length > 0) {
        // Build a cache key from the page's HTML text + stylesheet config.
        // Pages with identical content and CSS will produce identical
        // fontTracer results, so we can skip the expensive tracing.
        const stylesheetKey = getStylesheetCacheKey(htmlOrSvgAsset);
        const contentHash = crypto
          .createHash('md5')
          .update(htmlOrSvgAsset.text || '')
          .update('\x1d')
          .update(stylesheetKey)
          .digest('hex');
        ...
        if (cachedResult && !headlessBrowser) {
          ...
          textByProps = cachedResult.map((entry) => ({ ...entry }));
        } else {
