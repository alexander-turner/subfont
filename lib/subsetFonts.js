      ...
      if (!uniqueSubsetBuffers.has(subsetBuffer)) {
        uniqueSubsetBuffers.set(
          ...
          getFontInfo(subsetBuffer)
            ...
            .catch((err) => {
              console.warn(
                `subfont: Failed to parse subset font: ${err.message}`
              );
              return new Set();
            })
        );
...
async function subsetFonts(
  ...
  await warnAboutMissingGlyphs(htmlOrSvgAssetTextsWithProps, assetGraph, console);
  debugLog(
    console,
    debug,
  ...
  for (const htmlOrSvgAsset of htmlOrSvgAssets) {
    const accumulatedFontFaceDeclarations =
      fontFaceDeclarationsByHtmlOrSvgAsset.get(htmlOrSvgAsset);
    // Font face declarations are not grouped by media query here because
    // @font-face rules are not conditional — browsers preload them regardless
    // of media query matches. Grouping would add complexity without benefit.
