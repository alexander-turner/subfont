  ...
  try {
    ...
    if (fastPathPages.length > 0) {
      ...
      for (const pd of fastPathPages) {
        ...
        const { uniquePropsMap, textPerPropsKey, seenVariantKeys } =
          getRepData(pd.representativePd);
      ...
      debugLog(
        console,
        debug,
        `[subfont timing] Fast-path text extraction (${fastPathPages.length} pages, ${fastPathFallbacks} fell back to full trace): ${
