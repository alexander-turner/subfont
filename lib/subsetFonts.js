...
async function collectTextsByPage(
  ...
  function getOrComputeStylesheetResults(htmlOrSvgAsset) {
    ...
    // font-variant-* CSS properties. Walk the parsed CSS AST for each
    // stylesheet, checking each rule for these properties and extracting
    // the font-family from the same rule. If no font-family is declared
    // in the same rule (i.e., it's inherited), we conservatively assume
    ...
    for (const swp of stylesheetsWithPredicates) {
      if (!swp.asset || !swp.asset.parseTree) continue;
      swp.asset.parseTree.walkRules((rule) => {
        if (fontFamiliesWithFeatureSettings === true) return;
        let hasFeatureProp = false;
        let fontFamilyValue = null;
        rule.walkDecls((decl) => {
          const prop = decl.prop.toLowerCase();
          if (featureSettingsProps.has(prop)) {
            hasFeatureProp = true;
          } else if (prop === 'font-family') {
            fontFamilyValue = decl.value;
          }
        });
        ...
        if (fontFamilyValue) {
          ...
          const families = cssFontParser
            .parseFontFamily(fontFamilyValue)
            .map((f) => f.toLowerCase());
          for (const family of families) {
            fontFamiliesWithFeatureSettings.add(family);
          }
        } else {
          // No font-family in this rule — conservatively mark all fonts
