...
async function warnAboutMissingGlyphs(
  ...
  // Cache missing glyph check results per (subsetBuffer, pageText) pair.
  // Since pageText is uniqueChars output, many pages share identical
  // character sets per font, making this check redundant across pages.
  const missingCheckCache = new Map();
  ...
  for (const {
    ...
    for (const fontUsage of fontUsages) {
      if (fontUsage.subsets) {
        ...
        // Check if we already verified this exact (font, pageText) combo
        const checkKey = `${subsetCharSetCache.size}\x1d${fontUsage.pageText}`;
        const previousResult = missingCheckCache.get(checkKey);
        if (previousResult === false) {
          // No missing glyphs for this combo — skip
          continue;
        }
        ...
        missingCheckCache.set(checkKey, missedAny);
        if (missedAny) {
...
async function subsetFonts(
  ...
  // Cache getCodepoints results by pageText string to avoid recomputing
  // for pages with identical text content per font (common on template sites).
  const pageCodepointsCache = new Map();
  ...
  for (const htmlOrSvgAssetTextWithProps of htmlOrSvgAssetTextsWithProps) {
    for (const fontUsage of htmlOrSvgAssetTextWithProps.fontUsages) {
      ...
      if (cached.originalCodepoints) {
        let pageCodepoints = pageCodepointsCache.get(fontUsage.pageText);
        if (!pageCodepoints) {
          pageCodepoints = getCodepoints(fontUsage.pageText);
          pageCodepointsCache.set(fontUsage.pageText, pageCodepoints);
        }
