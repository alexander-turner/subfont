...
async function subsetFonts(
  ...
  if (console) {
    ...
    for (const item of htmlOrSvgAssetTextsWithProps) {
      for (const fu of item.fontUsages) {
        if (fu.fontUrl) uniqueFontUrls.add(fu.fontUrl);
      }
    }
    if (uniqueFontUrls.size > 0) {
      console.log(
        `  Subsetting ${uniqueFontUrls.size} unique font file${uniqueFontUrls.size === 1 ? '' : 's'}...`
      );
    }
  }
  ...
  const insertTotal = htmlOrSvgAssetTextsWithProps.length;
  const insertStep = Math.max(1, Math.ceil(insertTotal / 10));
  const showInsertProgress = debug && console && insertTotal >= 5;
  let insertIndex = 0;
  if (showInsertProgress) {
    console.log(
      `[subfont timing] Inserting subset CSS into ${insertTotal} page${insertTotal === 1 ? '' : 's'}...`
    );
  }
  for (const {
    htmlOrSvgAsset,
    ...
    if (
      showInsertProgress &&
      insertIndex % insertStep === 0 &&
      insertIndex < insertTotal
    ) {
      console.log(
        `[subfont timing]   inserting subsets: ${insertIndex}/${insertTotal} pages...`
      );
    }
