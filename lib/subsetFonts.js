...
async function collectTextsByPage(
  ...
  function getFontFaceGroupKey(declarations) {
    ...
    return declarations
      .map(
        (d) =>
          `${d['font-family']}/${d['font-style']}/${d['font-weight']}/${
            d['font-stretch']
          }/${d.src || ''}`
      )
      .join('\x1d');
...
async function subsetFonts(
  ...
  for (const htmlOrSvgAsset of htmlOrSvgAssets) {
    const accumulatedFontFaceDeclarations =
      fontFaceDeclarationsByHtmlOrSvgAsset.get(htmlOrSvgAsset);
    // Grouping @font-face declarations by media query is not needed here.
    // The fallback CSS is generated per-page and each @font-face rule is
    // already scoped by the media context of its containing stylesheet
    // (tracked via getCssRulesByProperty's activeCssQueryPredicates).
    // Grouping would add complexity without benefit since browsers already
    // apply @font-face declarations only when the associated media matches.
