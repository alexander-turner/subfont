...
class FontTracerPool {
  constructor(numWorkers, cssRulesCache) {
    this._workerPath = pathModule.join(__dirname, 'fontTracerWorker.js');
    this._numWorkers = numWorkers;
    this._cssRulesCache = cssRulesCache;
    this._workers = [];
    this._idle = [];
    this._pendingTasks = [];
  ...
  async init() {
    ...
    for (let i = 0; i < this._numWorkers; i++) {
      ...
      // Send init with optional pre-computed CSS rules cache
      worker.postMessage({
        type: 'init',
        cssRulesCache: this._cssRulesCache || null,
      });
...
async function collectTextsByPage(
  ...
  const useWorkerPool = !headlessBrowser && pagesToTrace.length >= 4;

  try {
    if (useWorkerPool) {
      // Pre-compute getCssRulesByProperty results for shared stylesheets.
      // All pages typically share the same CSS, so we compute once and
      // pass to workers to avoid redundant PostCSS parsing.
      const cssRulesCacheEntries = [];
      const seenCssTexts = new Set();
      const defaultStylesheets = require('font-tracer/lib/defaultStylesheets');
      const propsToReturn = [
        'font-family',
        'font-style',
        'font-weight',
      ...
      for (const pd of pagesToTrace) {
        const allStylesheets = [...defaultStylesheets, ...pd.stylesheetsWithPredicates];
        for (const swp of allStylesheets) {
          const cacheKey = JSON.stringify([cssPropsToTrace, swp.text, swp.predicates]);
          if (!seenCssTexts.has(cacheKey)) {
            seenCssTexts.add(cacheKey);
            const result = memoizedGetCssRulesByProperty(
              cssPropsToTrace,
              swp.text,
              swp.predicates
            );
            cssRulesCacheEntries.push([cacheKey, result]);
          }
        }
      }

      const numWorkers = Math.min(os.cpus().length, pagesToTrace.length);
      const pool = new FontTracerPool(numWorkers, cssRulesCacheEntries);
      await pool.init();

      try {
        // Dispatch all pages to the worker pool in parallel
        const tracePromises = pagesToTrace.map(async (pd) => {
          try {
      ...
      // Sequential path: few pages or dynamic mode
      for (const pd of pagesToTrace) {
        pd.textByProps = fontTracer(pd.htmlOrSvgAsset.parseTree, {
          stylesheetsWithPredicates: pd.stylesheetsWithPredicates,
          getCssRulesByProperty: memoizedGetCssRulesByProperty,
    ...
    // Assemble results from both cached and freshly-traced pages
    const allPageData = [...pagesFromCache, ...pagesToTrace];
