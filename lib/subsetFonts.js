  ...
  for (const {
    ...
    for (const fontUsage of fontUsages) {
      if (fontUsage.subsets) {
        ...
        let characterSet = subsetCharSetCache.get(subsetBuffer);
        if (!characterSet) {
          characterSet = (await getFontInfo(subsetBuffer)).characterSet;
          subsetCharSetCache.set(subsetBuffer, characterSet);
        }
        // Convert to Set for O(1) lookups instead of O(n) array.includes
        const characterSetLookup = new Set(characterSet);
...
async function collectTextsByPage(
  ...
  const stylesheetRelTypes = new Set([
    'HtmlStyle',
    'SvgStyle',
    'CssImport',
  ...
  function getOrComputeStylesheetResults(htmlOrSvgAsset) {
    ...
    const accumulatedFontFaceDeclarations = [];
    {
      ...
      const traversalTypes = new Set(['HtmlStyle', 'SvgStyle', 'CssImport']);
      const inlineHtmlCheck = (rel) =>
      ...
      (function traverseForFontFace(asset) {
        ...
        for (const rel of rels) {
          if (traversalTypes.has(rel.type) || inlineHtmlCheck(rel)) {
  ...
  console.log(`[subfont timing] Pages needing full trace: ${pagesNeedingFullTrace.length}, fast extract: ${pagesForFastExtract.length}, cached: ${pageData.length - pagesToTrace.length}`);
...
async function subsetFonts(
  ...
  let _sfPhaseStart = Date.now();
  if (!skipSourceMapProcessing) {
    await assetGraph.applySourceMaps({ type: 'Css' });
  }
  console.log(`[subfont timing] applySourceMaps: ${Date.now() - _sfPhaseStart}ms`);

  _sfPhaseStart = Date.now();
  await assetGraph.populate({
    followRelations: {
      $or: [
        {
  ...
  console.log(`[subfont timing] populate (google fonts): ${Date.now() - _sfPhaseStart}ms`);
