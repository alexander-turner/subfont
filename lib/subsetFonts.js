...
async function warnAboutMissingGlyphs(
  ...
  for (const {
    ...
    for (const fontUsage of fontUsages) {
      ...
      let missedAny = false;
      const lineIndex = new LinesAndColumns(htmlOrSvgAsset.text);
      for (const char of fontUsage.pageText) {
        ...
        if (isMissing) {
          ...
          if (charIdx === -1) {
            ...
            const position = lineIndex.locationForIndex(charIdx);
      ...
      fontInfoPromises.set(
        ...
        getFontInfo(fontAsset.rawSrc).catch((err) => {
          const warning = new Error(
            `Failed to parse font ${fontUrl}: ${err.message}`
          );
          warning.asset = fontAsset;
          assetGraph.warn(warning);
          return null;
        })
      );
    }
