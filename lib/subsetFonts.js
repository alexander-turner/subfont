...
async function collectTextsByPage(
  ...
  // Traverse stylesheet relations from an HTML/SVG asset, building a
  // cache key from asset identity and relation context. The optional
  // `shouldSkip` predicate can exclude specific CSS assets from the key.
  const _fontRelevantRegex =
    /font-family|font-weight|font-style|font-stretch|font-display|@font-face|font-variation|font-feature/i;

  function buildStylesheetKey(htmlOrSvgAsset, shouldSkip) {
    const keyParts = [];
    ...
    (function traverse(asset, isNoscript) {
      ...
      for (const relation of rels) {
        if (relation.type === 'HtmlNoscript') {
          ...
          if (shouldSkip && shouldSkip(relation.to)) {
            continue;
          }
  ...
  function getStylesheetCacheKey(htmlOrSvgAsset) {
    return buildStylesheetKey(htmlOrSvgAsset);
  }

  // Grouping key for fast-path optimization. Skips inline stylesheets
  // that don't contain font-related CSS properties. Non-font-relevant
  // inline CSS (e.g., critical CSS for layout injected per-page by site
  // generators) would otherwise give every page a unique key even though
  // font-tracer output is identical across those pages.
  function getStylesheetGroupKey(htmlOrSvgAsset) {
    return buildStylesheetKey(
      htmlOrSvgAsset,
      (asset) =>
        asset.isInline &&
        asset.type === 'Css' &&
        !_fontRelevantRegex.test(asset.text || '')
    );
  }
  ...
  for (const htmlOrSvgAsset of htmlOrSvgAssets) {
    ...
    pageData.push({
      ...
      stylesheetsWithPredicates,
      fontFamiliesWithFeatureSettings,
      stylesheetCacheKey: getStylesheetGroupKey(htmlOrSvgAsset),
    });
  }
    ...
    if (fontAsset.isLoaded) {
      fontInfoPromises.set(
        ...
        getFontInfo(fontAsset.rawSrc).catch((err) => {
          if (console) {
            console.warn(
              `subfont: Failed to parse font ${fontUrl}: ${err.message}`
            );
          }
          return null;
        })
