...
async function collectTextsByPage(
  ...
  function getOrComputeStylesheetResults(htmlOrSvgAsset) {
    ...
    // Detect which font-families are affected by font-feature-settings or
    // font-variant-* CSS properties. We parse each stylesheet to find CSS
    // rule blocks that contain these properties, then extract the font-family
    // from the same block. If no font-family is specified in the same rule
    // (i.e., it's inherited), we conservatively assume all fonts are affected.
    // This avoids the wasteful per-page boolean that triggers expensive glyph
    // collection for every font when only one font-family actually needs it.
    const featureSettingsPattern = /font-feature-settings\s*:/i;
    const fontVariantPattern =
      /font-variant-(alternates|caps|east-asian|ligatures|numeric|position)\s*:/i;

    let fontFamiliesWithFeatureSettings = null; // null = not computed yet
    for (const swp of stylesheetsWithPredicates) {
      if (
        !featureSettingsPattern.test(swp.text) &&
        !fontVariantPattern.test(swp.text)
      ) {
        continue;
      }
      // Parse the CSS to find rules with feature settings and their font-families
      const asset = swp.asset || swp;
      if (asset.parseTree) {
        if (!fontFamiliesWithFeatureSettings) {
          fontFamiliesWithFeatureSettings = new Set();
        }
        let allFontsAffected = false;
        asset.parseTree.walkDecls((decl) => {
          if (allFontsAffected) return;
          if (
            decl.prop &&
            (featureSettingsPattern.test(`${decl.prop}:`) ||
              fontVariantPattern.test(`${decl.prop}:`))
          ) {
            // Find the font-family in the same rule
            const parentRule = decl.parent;
            if (parentRule) {
              let foundFamily = false;
              parentRule.walkDecls('font-family', (familyDecl) => {
                const families = familyDecl.value.split(',').map((f) =>
                  f
                    .trim()
                    .replace(/^['"]|['"]$/g, '')
                    .toLowerCase()
                ...
                for (const family of families) {
                  fontFamiliesWithFeatureSettings.add(family);
                }
                foundFamily = true;
              });
              if (!foundFamily) {
                // No font-family in the same rule — conservatively mark all fonts
                allFontsAffected = true;
              }
            }
          }
        });
        ...
        // Can't parse — conservatively assume all fonts affected
        fontFamiliesWithFeatureSettings = true;
        break;
      }
    }
  ...
  for (const pd of pageData) {
    // Every page needs full tracing because the fast extraction
    // optimization is incorrect: it reuses the template page's
    // traced font properties, but different pages may have different
    // HTML elements that trigger different font variants (e.g.,
    // page 1 has no <strong>, page 2 does → bold variant is missed).
  ...
  debugLog(
    console,
    debug,
    `[subfont timing] Pages needing full trace: ${pagesNeedingFullTrace.length}, fast extract: ${pagesForFastExtract.length}`
  );
  ...
  try {
    ...
    debugLog(
      console,
      debug,
      `[subfont timing] Fast extraction (${
        pagesForFastExtract.length
      } pages): ${Date.now() - fastExtractStart}ms`
    );
      ...
      fontInfo: htmlOrSvgAssetTextsWithProps.map(
        ({ fontUsages, htmlOrSvgAsset }) => ({
          ...
          fontUsages: fontUsages.map((fontUsage) =>
            _.omit(fontUsage, 'hasFontFeatureSettings')
          ),
        })
      ),
    };
  ...
  return {
    fontInfo: htmlOrSvgAssetTextsWithProps.map(
      ({ fontUsages, htmlOrSvgAsset }) => ({
        assetFileName: htmlOrSvgAsset.nonInlineAncestor.urlOrDescription,
        fontUsages: fontUsages.map((fontUsage) =>
          _.omit(fontUsage, 'subsets', 'hasFontFeatureSettings')
        ),
      })
    ),
  };
