 ...
 * content attributes. It strips script/style/svg element contents
 * and decodes HTML entities.
 */
function extractVisibleText(html) {
  // Remove script, style, and SVG elements with their contents
  let text = html.replace(/<(script|style|svg)\b[^>]*>[\s\S]*?<\/\1\s*>/gi, ' ');
...
class FontTracerPool {
  ...
  _onWorkerExit(worker, code) {
    if (code !== 0) {
      // Worker crashed — reject all pending tasks assigned to it and
      // reject any task that was in-flight (its callback is still in the map).
      // Since we can't tell which specific task was assigned to which worker,
      // remove this worker from the idle pool so it's never reused.
      const idx = this._idle.indexOf(worker);
      if (idx !== -1) {
        this._idle.splice(idx, 1);
      }
      // Reject all remaining callbacks — they'll fall back to main thread
      for (const [taskId, cb] of this._taskCallbacks) {
...
async function collectTextsByPage(
  ...
  // Pre-compute stylesheet results for all pages and build content hashes
  const pageData = [];
  for (const htmlOrSvgAsset of htmlOrSvgAssets) {
    ...
    // Build content hash for the fontTracer result cache
    const contentHasher = crypto
      .createHash('md5')
      .update(htmlOrSvgAsset.text || '')
      .update('\x1d');
    for (const swp of stylesheetsWithPredicates) {
      if (swp.text) {
        contentHasher.update(swp.text);
      }
  ...
  // Determine which pages need tracing vs can use cache.
  // pageData preserves the original htmlOrSvgAssets order throughout.
  const pagesToTrace = [];

  for (const pd of pageData) {
    const cachedResult = fontTracerResultCache.get(pd.contentHash);
    if (cachedResult && !headlessBrowser) {
      pd.textByProps = cachedResult.map((entry) => ({
        ...entry,
        props: { ...entry.props },
      }));
    } else {
      pagesToTrace.push(pd);
    }
  }

  // Template-aware tracing optimization:
  // Group pages by their stylesheet configuration key. For pages sharing
  // the same CSS, font-tracer produces entries with the same `props`
  // (same CSS cascade → same font assignments per DOM structure). Only
  // the text content differs. We trace the first page in each group
  // with full font-tracer, then extract text from remaining pages using
  // fast regex-based extraction and reuse the same props structure.
  //
  // This reduces font tracing from O(pages × jsdom+CSS) to
  // O(groups × jsdom+CSS + pages × regex).
  //
  // We only use fast extraction for groups with enough pages to justify
  // the approximation. Small groups (< FAST_EXTRACT_THRESHOLD) use full
  // tracing for maximum accuracy.
  const FAST_EXTRACT_THRESHOLD = 5;
  const stylesheetGroupSizes = new Map();
  for (const pd of pagesToTrace) {
    const key = getStylesheetCacheKey(pd.htmlOrSvgAsset);
    stylesheetGroupSizes.set(key, (stylesheetGroupSizes.get(key) || 0) + 1);
  }
  ...
  for (const pd of pagesToTrace) {
    const stylesheetKey = getStylesheetCacheKey(pd.htmlOrSvgAsset);
    const groupSize = stylesheetGroupSizes.get(stylesheetKey);

    if (!pagesByStylesheetKey.has(stylesheetKey)) {
      // First page with this CSS config — always needs full tracing
      pagesByStylesheetKey.set(stylesheetKey, pd);
      pagesNeedingFullTrace.push(pd);
    } else if (groupSize < FAST_EXTRACT_THRESHOLD) {
      // Small group — use full tracing for accuracy
      pagesNeedingFullTrace.push(pd);
    } else {
      // Large group — use fast extraction for subsequent pages
      pd.templatePage = pagesByStylesheetKey.get(stylesheetKey);
  ...
  try {
    if (useWorkerPool) {
      ...
      // Cache results from worker-traced pages
      for (const pd of pagesNeedingFullTrace) {
        if (!fontTracerResultCache.has(pd.contentHash)) {
          fontTracerResultCache.set(
            pd.contentHash,
            pd.textByProps.map((entry) => ({
              ...entry,
              props: { ...entry.props },
            }))
          );
