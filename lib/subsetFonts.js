...
// Escape a value for safe inclusion in a single-quoted JS string literal.
// Uses JSON.stringify for robust escaping of backslashes, quotes, newlines,
// U+2028, U+2029, etc., then strips the outer double quotes.
// The < escape prevents </script> from closing the inline script tag.
function escapeJsStringLiteral(str) {
  // JSON.stringify returns a double-quoted string with all specials escaped.
  // Slice off the quotes, then escape ' (not escaped by JSON) and < (for HTML safety).
...
async function collectTextsByPage(
  ...
  //
  // When `skipNonFontInlineCss` is true, inline stylesheets without
  // font-related properties are excluded from the key. This lets the
  // fast-path group pages that differ only in layout-only inline CSS.
  ...
  function getOrComputeStylesheetResults(htmlOrSvgAsset) {
    ...
    // We keep every declaration so that the subsetting pipeline can produce a
    // separate subset font per unicode-range partition.
    if (accumulatedFontFaceDeclarations.length > 0) {
      ...
      for (const [comboKey, group] of comboGroups) {
        if (group.length > 1) {
          // Ensure every member specifies a unicode-range (otherwise the
          // overlap is ambiguous and we can't meaningfully split).
  ...
  for (const htmlOrSvgAsset of htmlOrSvgAssets) {
    ...
    pageData.push({
      ...
      stylesheetsWithPredicates,
      fontFamiliesWithFeatureSettings,
      stylesheetCacheKey: buildStylesheetKey(htmlOrSvgAsset, true),
    });
  }
  ...
  try {
    ...
    debugLog(
      console,
      debug,
      `[subfont timing] Full tracing (${pagesNeedingFullTrace.length} pages): ${(subTimings['Full tracing'] = Date.now() - tracingStart)}ms`
    );
    ...
    if (fastPathPages.length > 0) {
      ...
      // Pre-compute per-representative data once (shared across all
      // fast-path pages in the same group).
      ...
      function getRepData(representativePd) {
        ...
        // Collect unique props and per-props text from representative.
        // The per-props text includes CSS content (::before/::after) which
        // extractVisibleText cannot see.
        const uniquePropsMap = new Map();
        const textPerPropsKey = new Map();
        // Track seen variants by (normalized family, weight, style, stretch)
        // so we can detect @font-face variants not used on the representative.
        // Font-tracer outputs computed font-family stacks (e.g. "'jetbrain', serif"),
        // so we extract individual family names for matching against @font-face.
        ...
        for (const entry of repTextByProps) {
          ...
          if (family) {
            ...
            for (const fam of cssFontParser.parseFontFamily(family)) {
              seenVariantKeys.add(
                `${fam.toLowerCase()}\x1d${weight}\x1d${style}\x1d${stretch}`
              );
            }
          }
        }
      ...
      for (const pd of fastPathPages) {
        // If this page has inline style attributes with font properties,
        // the representative's props may not cover it. Fall back to full trace.
        ...
        // Include @font-face variants not seen in the representative's trace.
        // A variant is (family, weight, style, stretch). This catches fonts
        // like bold (same family, different weight) that the representative
        // didn't use, not just entirely unseen family names.
        const extendedPropsMap = new Map(uniquePropsMap);
        const extendedTextPerPropsKey = new Map(textPerPropsKey);
        for (const decl of pd.accumulatedFontFaceDeclarations) {
          ...
          const variantKey = `${family.toLowerCase()}\x1d${weight}\x1d${style}\x1d${stretch}`;
          if (!seenVariantKeys.has(variantKey)) {
            const propsKey = `${family}\x1d${weight}\x1d${style}\x1d${stretch}`;
            if (!extendedPropsMap.has(propsKey)) {
              extendedPropsMap.set(propsKey, {
                'font-family': family,
                'font-weight': weight,
                'font-style': style,
                'font-stretch': stretch,
              });
              extendedTextPerPropsKey.set(propsKey, []);
            }
          }
        }
        ...
        // Create textByProps entries. For each props, include:
        // 1. This page's own DOM text (from extractVisibleText)
        // 2. The representative's text for that props (preserves CSS
        //    content from ::before/::after which extractVisibleText misses)
        pd.textByProps = [];
        for (const [propsKey, props] of extendedPropsMap) {
          const repTexts = extendedTextPerPropsKey.get(propsKey) || [];
          pd.textByProps.push({
      ...
      debugLog(
        console,
        debug,
        `[subfont timing] Fast-path text extraction (${fastPathPages.length} pages, ${fastPathFallbacks} fell back to full trace): ${(subTimings['Fast-path extraction'] = Date.now() - fastPathStart)}ms`
      );
    }
  ...
  function getDeclarationsKey(declarations) {
    return declarations
      .map(
        (d) =>
          `${d['font-family']}/${d['font-style']}/${d['font-weight']}/${d['font-stretch']}/${d['unicode-range'] || '*'}`
      )
      .join('\x1d');
  }
    ...
    for (const textAndProps of globalTextByProps) {
      ...
      if (!snappedResults) {
        ...
        for (const fam of families) {
          ...
          // Find sibling declarations: same family/style/weight/stretch but
          // different unicode-range (e.g. CJK fonts split across ranges).
          const siblingDeclarations = declarations.filter(
            (d) =>
              d['font-family'].toLowerCase() === fam.toLowerCase() &&
              d['font-style'] === activeFontFaceDeclaration['font-style'] &&
              d['font-weight'] === activeFontFaceDeclaration['font-weight'] &&
              d['font-stretch'] === activeFontFaceDeclaration['font-stretch'] &&
              d['unicode-range'] &&
              d !== activeFontFaceDeclaration
          );

          const declsToProcess =
            siblingDeclarations.length > 0 &&
            activeFontFaceDeclaration['unicode-range']
              ? [activeFontFaceDeclaration, ...siblingDeclarations]
              : [activeFontFaceDeclaration];

          for (const decl of declsToProcess) {
            const {
              relations: declRelations,
              '-subfont-text': _subfontText,
              ...declProps
            } = decl;
            const declFontUrl = getPreferredFontUrl(declRelations);
            if (!declFontUrl) {
              continue;
            }
...
async function subsetFonts(
  ...
  timings.warnAboutUnusedVariationAxes = Date.now() - phaseStart;
  debugLog(
    console,
    debug,
    `[subfont timing] warnAboutUnusedVariationAxes: ${timings.warnAboutUnusedVariationAxes}ms`
  );
  phaseStart = Date.now();
  ...
  debugLog(
    console,
    debug,
    `[subfont timing] insert subsets loop: ${(timings['insert subsets loop'] = Date.now() - phaseStart)}ms`
  );
  phaseStart = Date.now();
  ...
  debugLog(
    console,
    debug,
    `[subfont timing] lazy load fallback CSS: ${(timings['lazy load fallback CSS'] = Date.now() - phaseStart)}ms`
  );
  phaseStart = Date.now();
  ...
  debugLog(
    console,
    debug,
    `[subfont timing] remove original @font-face: ${(timings['remove original @font-face'] = Date.now() - phaseStart)}ms`
  );
  phaseStart = Date.now();
  ...
  debugLog(
    console,
    debug,
    `[subfont timing] Google Fonts + cleanup: ${(timings['Google Fonts + cleanup'] = Date.now() - phaseStart)}ms`
  );
  phaseStart = Date.now();
  ...
  debugLog(
    console,
    debug,
    `[subfont timing] inject subset font-family into CSS/SVG: ${(timings['inject subset font-family'] = Date.now() - phaseStart)}ms`
  );
  phaseStart = Date.now();
  ...
  debugLog(
    console,
    debug,
    `[subfont timing] source maps + orphan cleanup: ${(timings['source maps + orphan cleanup'] = Date.now() - phaseStart)}ms`
  );
