...
async function collectTextsByPage(
  ...
  function getOrComputeStylesheetResults(htmlOrSvgAsset) {
    ...
    // Detect if any stylesheet uses font-feature-settings or font-variant-*
    // (other than font-variant: normal). When detected, font subsetting must
    // include all original codepoints so that GSUB alternate glyphs are
    // preserved through HarfBuzz layout closure.
    const hasFontFeatureSettings = stylesheetsWithPredicates.some(
      (swp) =>
        /font-feature-settings\s*:/i.test(swp.text) ||
        /font-variant-(alternates|caps|east-asian|ligatures|numeric|position)\s*:/i.test(
          swp.text
        )
    );
  ...
  try {
    ...
    for (const pd of pageData) {
      ...
      htmlOrSvgAssetTextsWithProps.push({
        ...
        hasFontFeatureSettings: pd.hasFontFeatureSettings,
      });
    }
    ...
    htmlOrSvgAssetTextsWithPropsEntry.fontUsages = fontUsageTemplates.map(
      (template) => {
        ...
        return {
          ...
          hasFontFeatureSettings: hasFontFeatureSettings || false,
        };
      }
    );
