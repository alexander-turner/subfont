...
async function subsetFonts(
  ...
  if (htmlOrSvgAssetTextsWithProps.length > 0) {
    const totalPages = htmlOrSvgAssetTextsWithProps.length;
    const pageCountByFontUrl = new Map();
    for (const { fontUsages } of htmlOrSvgAssetTextsWithProps) {
      // Use a Set to count each fontUrl at most once per page
      const seenOnPage = new Set();
      for (const fu of fontUsages) {
        if (fu.pageText && !seenOnPage.has(fu.fontUrl)) {
          seenOnPage.add(fu.fontUrl);
          pageCountByFontUrl.set(
            fu.fontUrl,
            (pageCountByFontUrl.get(fu.fontUrl) || 0) + 1
          );
        }
      }
    }
    for (const [fontUrl, count] of pageCountByFontUrl) {
      if (count === totalPages) {
