...
async function collectTextsByPage(
  ...
  // Grouping key for fast-path optimization. Uses asset IDs (like the
  // cache key) but skips inline stylesheets that don't contain any
  // font-related CSS properties. Non-font-relevant inline CSS (e.g.,
  // critical CSS for layout injected per-page by site generators) would
  // otherwise give every page a unique key even though font-tracer
  // output is identical across those pages.
    ...
    (function traverse(asset, isNoscript) {
      ...
      for (const relation of rels) {
        if (relation.type === 'HtmlNoscript') {
          ...
          // Skip inline stylesheets that don't contain font-related CSS.
          // These can't affect fontTracer output and would cause pages
          // with different layout-only inline styles to get separate keys.
          if (
            target.isInline &&
            target.type === 'Css' &&
            !_fontRelevantRegex.test(target.text || '')
          ) {
            continue;
          }
          const media = relation.media || '';
