...
async function collectTextsByPage(
  assetGraph,
  htmlOrSvgAssets,
  { text, console, dynamic = false, fast = false, debug = false } = {}
) {
  ...
  const fastPathPages = [];
  if (!fast || headlessBrowser) {
    // Without --fast, or in dynamic mode (headlessBrowser.tracePage produces
    // page-specific JS-dependent results), trace all pages fully.
  ...
  try {
    ...
    if (fastPathPages.length > 0) {
      ...
      function getRepData(representativePd) {
        ...
        const data = { uniquePropsMap, textPerPropsKey, seenFontFamilies };
      ...
      for (const pd of fastPathPages) {
        ...
        const { uniquePropsMap, textPerPropsKey, seenFontFamilies } =
