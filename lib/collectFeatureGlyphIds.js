 ...
 */
// Serialization queue: harfbuzzjs WASM may return corrupt results when
// multiple calls run concurrently on the shared module instance.  This
// ensures only one WASM operation runs at a time, matching the approach
// used by getFontInfo.
...
function collectFeatureGlyphIds(fontBuffer, text) {
  ...
  return (wasmQueue = wasmQueue.then(
    () => collectFeatureGlyphIdsImpl(fontBuffer, text),
    () => collectFeatureGlyphIdsImpl(fontBuffer, text)
  ));
}
