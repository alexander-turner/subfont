...
function parseFontStretchRange(str) {
  ...
  const fontStretchTokens = str.split(/\s+/).map((str) => {
    const normalized = normalizeFontPropertyValue('font-stretch', str);
    return typeof normalized === 'number' ? normalized : parseFloat(normalized);
  });
  if (
