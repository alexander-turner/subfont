import cssFontWeightNames = require('css-font-weight-names');
import initialValueByProp = require('./initialValueByProp');
import unquote = require('./unquote');
import normalizeFontStretch = require('font-snapper/lib/normalizeFontStretch');
...
function applyBolder(weight: number): number {
...
function applyLighter(weight: number): number {
...
function resolveWeightToken(token: string): number {
...
function normalizeFontPropertyValue(
  propName: string,
  ...
  if (propNameLowerCase === 'font-family') {
    ...
    const numericWeight = resolveWeightToken(String(parsedValue));
    if (isValidWeight(numericWeight)) {
      return numericWeight;
    }
    return value;
  } else if (propNameLowerCase === 'font-stretch') {
    return normalizeFontStretch(String(value));
