...
async function collectFeatureGlyphIdsImpl(
  ...
  try {
    const fontFeatures = new Set<string>(face.getTableFeatureTags('GSUB'));
    ...
    const allowedTags =
      cssTagsArr.length > 0 ? new Set(cssTagsArr) : GSUB_FEATURE_TAGS;
    ...
    try {
      ...
      baseGids = new Set(baseGlyphs.map((g: { g: number }) => g.g));
    } finally {
      baseBuf.destroy();
    }

    const altGlyphIds = new Set<number>();
