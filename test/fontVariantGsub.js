...
// Inline text extraction — walks parse5 tree collecting text nodes,
// skipping invisible elements (script, style, svg, template).
function extractVisibleText(html) {
  const INVISIBLE = new Set(['script', 'style', 'svg', 'template']);
  const doc = parse5.parse(html);
  const parts = [];
  function walk(node) {
    if (node.nodeName && INVISIBLE.has(node.nodeName)) return;
